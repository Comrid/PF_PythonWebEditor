...
# =============================================================================
# PathFinder 로봇 설정 스크립트 v2.0
# 라즈베리파이 제로 2용 완전 자동화 설정
# =============================================================================
...
ROBOT_CONFIG_FILE="/home/${ACTUAL_USER}/robot_config.py"

# 서버 설정
SERVER_URL="https://pathfinder-kit.duckdns.org"
CANONICAL_HOSTNAME="pathfinder-kit.duckdns.org"

echo "🚀 PathFinder 로봇 설정을 시작합니다 (User=$ACTUAL_USER)"
echo "📱 라즈베리파이 제로 2용 최적화 버전"
sleep 2

# --- [1] 필수 패키지 설치 ---
echo "[1/15] 필수 패키지를 설치합니다..."
sudo apt-get update
echo "iptables-persistent iptables-persistent/autosave_v4 boolean true" | sudo debconf-set-selections
echo "iptables-persistent iptables-persistent/autosave_v6 boolean true" | sudo debconf-set-selections
sudo apt-get install -y hostapd dnsmasq git python3-pip python3-opencv gunicorn iptables-persistent iw iproute2 openssh-server python3-socketio python3-requests

# --- [2] 블루투스 비활성화 (라즈베리파이 제로 2 최적화) ---
echo "[2/15] 블루투스 기능을 비활성화합니다..."
grep -q "dtoverlay=disable-bt" /boot/config.txt || echo "dtoverlay=disable-bt" | sudo tee -a /boot/config.txt

# --- [3] 사용자 권한 설정 ---
echo "[3/15] 사용자($ACTUAL_USER)에게 네트워크 관리 권한을 부여합니다..."
sudo usermod -a -G netdev ${ACTUAL_USER}

# --- [4] Wi-Fi 하드웨어 준비 ---
echo "[4/15] Wi-Fi 국가 코드를 설정하고 하드웨어를 활성화합니다..."
sudo raspi-config nonint do_wifi_country KR
sudo rfkill unblock wifi
sudo systemctl unmask hostapd

# --- [5] GitHub 리포지토리 클론 ---
echo "[5/15] GitHub에서 최신 소스코드를 다운로드합니다..."
if [ -d "${CLONE_DIR}" ]; then
...
echo "[8/15] 가상 AP 인터페이스(uap0) 생성 서비스를 등록합니다..."
...
echo "[9/15] 가상 AP 인터페이스(uap0)에 고정 IP(10.42.0.1)를 할당합니다..."
...
echo "[10/15] dnsmasq와 hostapd를 설정합니다..."
...
echo "[11/15] Flask 앱 서비스를 등록합니다..."
# WiFi 설정 앱 서비스 (app_wifi.py) - AP 모드에서 실행
...
# 로봇 클라이언트 서비스 (robot_client.py) - 클라이언트 모드에서 실행
...
# --- [12] 로봇 설정 파일 생성 ---
echo "[12/15] 로봇 설정 파일을 생성합니다..."
sudo -u ${ACTUAL_USER} tee ${ROBOT_CONFIG_FILE} >/dev/null <<'CONFIG'
# PathFinder 로봇 설정 파일
...
# 네트워크 모드 (offline/online)
NETWORK_MODE = "offline"
CONFIG

# --- [13] 동적 모드 전환 스크립트 생성 ---
echo "[13/15] 동적 네트워크 모드 전환 스크립트를 생성합니다..."
...
WAN_IF="wlan0"
ROBOT_CONFIG="/home/pi/robot_config.py"
...
echo "[14/15] dhcpcd 훅과 sudoers 권한을 설정합니다..."
...
echo ""
echo "✅ PathFinder 로봇 설정이 완료되었습니다!"
echo ""
echo "📋 설정 완료 요약:"
