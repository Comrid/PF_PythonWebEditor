    ...
    try {
        if (window.mainGridStack) {
            ...
            if (widgetElement) {
                ...
                // 웹캠 위젯이면 생성 가능 수(카운터) 회복
                try {
                    const removedId = widgetElement.id || widgetId;
                    if (typeof removedId === 'string' && removedId.indexOf('webcamDisplayWidget_') === 0) {
                        if (typeof numWebcamDisplayWidget === 'number' && numWebcamDisplayWidget > 0) {
                            numWebcamDisplayWidget -= 1;
                        }
                        if (typeof window.disableHandGesture === 'function') {
                            window.disableHandGesture(removedId);
                        }
                    }
                } catch (e) { /* no-op */ }
            } else {
...
function createWidget_WebcamDisplay(){
    ...
    if (typeof maxWebcamCount === 'number' && numWebcamDisplayWidget >= maxWebcamCount) {
        showToast('생성 가능한 웹캠 위젯 수를 초과했습니다.', 'warning');
        return null;
    }

    const idx = numWebcamDisplayWidget;
    const widgetId = `webcamDisplayWidget_${idx}`;
            ...
            <div class="grid-stack-item-content widget-content">
                <div class="widget-header">
                    <h4>
                        ...
                        <span class="widget-title">Cam${idx}</span>
                    </h4>
                    <div class="widget-controls">
                        <select class="form-select" id="webcamDeviceSelector_${widgetId}"></select>
                    </div>
    ...
    createWidgetByHTML(widgetHTML);
    populateAndBindWebcamSelector(widgetId, idx);
    numWebcamDisplayWidget += 1;
    return widgetId;
}
...
function startWebcamStream(widgetId, deviceIndex){
    ...
    navigator.mediaDevices?.getUserMedia(constraints)
        ...
        .catch(err => {
            ...
            showToast('웹캠 접근에 실패했습니다. 브라우저 권한을 확인하세요.', 'error');
        });
}
...
function populateAndBindWebcamSelector(widgetId, defaultIndex){
    const selector = document.getElementById(`webcamDeviceSelector_${widgetId}`);
    if (!selector) return;
    ...
    // 초기 스트림 시작
    startWebcamStream(widgetId, defaultIndex);
